...
import {
  useSupplyChainChatStore,
  type Message,
  type ProgressStep,
} from '../stores/supplyChainChatStore';
import { downloadSession } from '../utils/chatExport';
import { isNearBottom } from '../utils/chatScroll';
import { getReportText } from '../utils/reportLanguage';

// 供应链专属快捷问题（取自 serenity-skill 的 research-prompt-pack / evals）
const QUICK_QUESTIONS = [
  '用 Serenity 方法分析 A 股 AI 半导体供应链，找优先研究的标的',
  '光模块产业链的瓶颈点在哪一层',
  '挑战一下：宁德时代是不是锂电池的核心卡点',
  '比较几家光通信设备商谁的卡点更强',
  '给中际旭创打供应链瓶颈分',
  '带我学 Serenity 供应链研究方法，每次问一个问题',
];

const SESSION_EXPORT_TITLE = '供应链分析会话';

const SupplyChainChatPage: React.FC = () => {
  ...
  const handleStartNewChat = useCallback(() => {
    ...
    useSupplyChainChatStore.getState().startNewChat();
  ...
  const confirmDelete = useCallback(() => {
    ...
    supplyChainApi
      ...
      .catch((error) => {
        console.error('Failed to delete supply chain session:', error);
      });
  ...
  const sidebarContent = (
    <>
      ...
      <ScrollArea testId="supply-chain-session-list-scroll" viewportClassName="p-3">
        {sessionsLoading ? (
  ...
  return (
    <div
      data-testid="supply-chain-workspace"
      ...
      <div className="flex h-full min-w-0 flex-1 flex-col overflow-hidden">
        <header className="mb-4 flex-shrink-0 space-y-3">
          ...
          <p className="text-secondary-text text-sm">
            基于 Serenity 供应链卡点方法（9 步深度调研）：先排产业链层级，再排公司，给出卡点、证据与证伪条件。
            <span className="ml-1 text-muted-text">深度调研模式，单次问答预计 5–10 分钟。</span>
          </p>
        </header>
        ...
        <div className="relative z-10 flex min-h-0 flex-1 flex-col overflow-hidden border border-white/6 bg-card/78 glass-card">
          ...
          <ScrollArea
            ...
            testId="supply-chain-message-scroll"
          >
            {messages.length === 0 && !loading ? (
              <div className="flex h-full items-center justify-center">
                <EmptyState
                  title="供应链分析"
                  description="用 Serenity 方法做供应链卡点调研：主题扫描、单公司挑战、候选对比、瓶颈打分。所有结论标注证据强度，不编造数据。"
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-end gap-3">
                <textarea
                  ...
                  placeholder="例如：分析光模块产业链瓶颈 / 挑战中际旭创是不是 CPO 核心 / 给 600519 打供应链分 (Enter 发送, Shift+Enter 换行)"
                  disabled={loading}
                  rows={1}
...
export default SupplyChainChatPage;
