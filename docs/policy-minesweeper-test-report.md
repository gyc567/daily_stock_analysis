...
| 模块 | 语句数 | 未覆盖 | 覆盖率 |
|------|-------|-------|-------|
| `src/services/policy_minesweeper_scorecard.py` | 116 | 0 | **100%** |
...
| **合计（可直接 scope 的 4 个模块）** | **456** | **0** | **100%** |
...
## 2. 后端测试用例清单（8 文件 / 174 用例，全过）

| 测试文件 | 用例数 | 覆盖重点 |
|---------|-------|---------|
| `test_policy_minesweeper_scorecard.py` | 35 | 六维加权 / 时间窗 blend / 5 档分级 / Markdown 渲染（含证据原文地址 url 链接）/ clamp / 防御分支 |
| `test_policy_minesweeper_scorecard_contract.py` | 4 | **icontract 契约（L2）**：score() `@ensure` 后置条件（final∈[-100,100]，极端输入+全 horizon） |
...
| `test_policy_minesweeper_api.py` | 19 | **Layer 3 Pydantic 422**（畸形 code/空 code/非法 horizon）/ agent 禁用 400 / done 事件 / error 事件 / list / get 404 / 路径穿越白名单 / delete / PDF |
...
- 全量后端套件中 PM 相关 174 用例全过；既有失败均为其他在研工作（P1–P3 scoring 缺 `runner` fixture、deep_research md2pdf `_font_registered` 等），非本模块引入。
