...
- [新功能] ocr (open-code-review) 集成到 Loop Engineering：新建 `.claude/skills/ocr-review/SKILL.md`（skill 封装 ocr review/scan/delegate）；`.claude/skills/ocr/SKILL.md`（ocr 自动安装 skill）；`docs/ocr-guide.md`（完整使用教程）；`.github/workflows/pr-review.yml` 的 `ai-review` job 替换为 `ocr review` + `gh pr comment`；`.github/workflows/loop-ci-sweeper.yml` 新增 `ocr-scan` job 对 CI 失败 PR 的变更文件做定向审计；`.github/workflows/loop-triage.yml` 新增 `ocr-review` job 对近 7 天变更文件做日常 review。
