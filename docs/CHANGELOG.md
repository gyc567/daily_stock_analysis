...
- [新功能] 趋势罗盘页新增「罗盘原理说明」（白话为主 + 关键技术参数括注，zh/en 双语，前端静态内联）：输入区下方默认折叠的五段式说明——模块定位与边界（不做无约束短期预测）、四层结构逐层解释（L0 仓位上限三档 / L1 年线 / L2 趋势段 / L3 节奏 + 阶段合成观察窗）、动作改写原则（约束只否决不升级 + 三个真实实例）、择时信号三形态与纪律（触发价/失效价/逆势博弈仓分离）、失效条件与数据纪律；动作改写节内含「全部 14 条硬约束速查表」二级折叠区（§4.5.2 全表白话化）。纯前端改动（TrendCompassPage + Collapsible 复用），无 API/schema 变更
