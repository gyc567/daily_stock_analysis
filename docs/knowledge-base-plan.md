...
| 将知识库定义为”最高优先级”数据源 | 用户材料代表专业判断，应优先引用 | **确认：用户知识库优先级最高，其他来源作为验证补充** |
...
知识库是"用户专业判断的核心证据库"，**优先级最高**。

在报告生成中的优先级如下：

| 优先级 | 来源 | 使用方式 |
| --- | --- | --- |
| **P0** | **用户知识库** | **用户私有材料，代表专业判断和投资论点，始终优先引用** |
| P1 | 交易所公告、巨潮公告、上市公司公告、定期报告 | 可作为确认事实 |
| P2 | 东方财富、同花顺、iFinD、AkShare 等结构化数据 | 可作为行情/财务/板块数据依据，需保留口径 |
| P3 | 公开新闻，社区、研报摘要 | 只能作为线索或市场分歧 |
| P4 | LLM 推断 | 只能作为待核验判断 |

**核心原则**：
- 用户上传的研究报告、行业分析、访谈纪要等材料代表用户的专业判断和投资论点
- 这些材料是投资决策的**主要证据**，必须始终优先引用
- 在引用其他来源（公告、行情等）之前，必须先引用知识库内容
- 其他公开来源只能作为补充验证或风险提示
