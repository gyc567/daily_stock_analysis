...
class CustomWebhookSender:
    def __init__(self, config: Config):
        ...
        self._custom_webhook_body_template = getattr(
            config, "custom_webhook_body_template", None
        )
        ...
        for i, url in enumerate(self._custom_webhook_urls):
            try:
                ...
                if self._is_dingtalk_webhook(url):
                    templated_payload = self._build_custom_webhook_template_payload(
                        content
                    )
                    if templated_payload is not None:
                        if self._post_custom_webhook(
                            url, templated_payload, timeout=30
                        ):
                            logger.info(f"自定义 Webhook {i + 1}（钉钉模板）推送成功")
                            success_count += 1
                        elif self._send_dingtalk_chunked(url, content, max_bytes=20000):
                            logger.info(
                                f"自定义 Webhook {i + 1}（钉钉模板失败，回退分批）推送成功"
                            )
                            success_count += 1
                        else:
        ...
        for i, url in enumerate(self._custom_webhook_urls):
            try:
                if self._is_discord_webhook(url):
                    ...
                    if fallback_content:
                        payload = self._build_custom_webhook_payload(
                            url, fallback_content
                        )
                        if self._post_custom_webhook(url, payload, timeout=30):
                            logger.info(
                                "自定义 Webhook %d（图片不支持，回退文本）推送成功",
                                i + 1,
                            )
                            success_count += 1
                    else:
    ...
    def test_custom_webhooks(
        self, content: str, *, timeout_seconds: float = 20.0
    ) -> List[Dict[str, Any]]:
    ...
    def _build_custom_webhook_template_payload(
        self, content: str
