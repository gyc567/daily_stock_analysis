...
class _TushareHttpClient:
    ...
    def __init__(
        self, token: str, timeout: int = 30, api_url: str = "http://api.tushare.pro"
    ) -> None:
        self._token = token
    ...
    def query(self, api_name: str, fields: str = "", **kwargs) -> pd.DataFrame:
        ...
        res = requests.post(
            self._api_url, json=cast(Any, req_params), timeout=self._timeout
        )
...
class TushareFetcher(BaseFetcher):
    ...
    name = "TushareFetcher"
    priority = int(
        os.getenv("TUSHARE_PRIORITY", "2")
    )  # 默认优先级，会在 __init__ 中根据配置动态调整
    ...
    def __init__(self, rate_limit_per_minute: int = 80):
        ...
        self.date_list: Optional[List[str]] = (
            None  # 交易日列表缓存（倒序，最新日期在前）
        )
    ...
    def _determine_priority(self) -> int:
        ...
        if config.tushare_token and self._api is not None:
            ...
            logger.info(
                "✅ 检测到 TUSHARE_TOKEN 且 API 初始化成功，Tushare 数据源优先级提升为最高 (Priority -1)"
            )
            return -1
    ...
    def _convert_stock_code(self, stock_code: str) -> str:
        ...
        if _is_us_code(raw_code):
            raise DataFetchError(
                f"TushareFetcher 不支持美股 {raw_code}，请使用 AkshareFetcher 或 YfinanceFetcher"
            )
    ...
    def get_stock_name(self, stock_code: str) -> Optional[str]:
        ...
        # 检查缓存
        stock_name_cache: Dict[str, str] = getattr(self, "_stock_name_cache", {})
        if stock_code in stock_name_cache:
            return stock_name_cache[stock_code]

        # 初始化缓存
        self._stock_name_cache = stock_name_cache
    ...
    def get_stock_list(self) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            df = self._api.stock_basic(
                exchange="", list_status="L", fields="ts_code,name,industry,area,market"
            )
            ...
            return cast(
                pd.DataFrame, df[["code", "name", "industry", "area", "market"]]
            )
            ...
            if use_realtime:
                try:
                    df = self._call_api_with_rate_limit(
                        "rt_k", ts_code="3*.SZ,6*.SH,0*.SZ,92*.BJ"
                    )
                    if df is not None and not df.empty:
                    ...
                    logger.error(
                        f"[Tushare] ts.pro_api().rt_k 尝试获取实时数据失败: {e}"
                    )
                    return None
            else:
                if current_date not in trade_dates:
                    last_date = self._pick_trade_date(
                        trade_dates, use_today=True
                    )  # 拿最近的日期
                else:
                    if current_clock < "09:30":
                        last_date = self._pick_trade_date(
                            trade_dates, use_today=False
                        )  # 拿取前一天的数据
                    else:  # 即 '> 16:30'
                        last_date = self._pick_trade_date(
                            trade_dates, use_today=True
                ...
                try:
                    ...
                    # 获取股票基础信息（包含代码和名称）
                    df_basic = self._call_api_with_rate_limit(
                        "stock_basic", fields="ts_code,name"
                    )
    ...
    def get_sector_rankings(self, n: int = 5) -> Optional[Tuple[list[Any], list[Any]]]:
        ...
        try:
            df = self._call_api_with_rate_limit(
                "moneyflow_ind_ths", trade_date=start_date
            )
            if df is not None and not df.empty:
        ...
        try:
            df = self._call_api_with_rate_limit(
                "moneyflow_ind_dc", trade_date=start_date
            )
            if df is not None and not df.empty:
    ...
    def get_chip_distribution(self, stock_code: str) -> Optional[ChipDistribution]:
        ...
        if _is_us_code(stock_code):
            logger.warning(
                f"[Tushare] TushareFetcher 不支持美股 {stock_code} 的筹码分布"
            )
            return None

        if _is_etf_code(stock_code):
            logger.warning(
                f"[Tushare] TushareFetcher 不支持 ETF {stock_code} 的筹码分布"
            )
            return None

        if _is_hk_market(stock_code):
            logger.warning(
                f"[Tushare] TushareFetcher 不支持港股 {stock_code} 的筹码分布"
            )
            return None
    ...
    def compute_cyq_metrics(
        ...
        # --- 获利比例 ---
        # 所有价格 <= 当前价的筹码之和
        winner_rate = df_sorted[df_sorted["price"] <= current_price][
            "norm_percent"
        ].sum()
        ...
        def get_percentile_price(target_pct: float) -> float:
            ...
            max_idx: int = int(len(df_sorted)) - 1
            clamped = int(idx) if int(idx) < max_idx else max_idx
            raw_price: Any = df_sorted.iloc[clamped]["price"]
