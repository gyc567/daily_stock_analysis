...
class AkshareFetcher(BaseFetcher):
    ...
    def _get_stock_realtime_quote_tencent(
        ...
        try:
            ...
            quote = UnifiedRealtimeQuote(
                ...
                circ_mv=(safe_float(fields[44]) or 0.0) * 100000000
                if len(fields) > 44 and fields[44]
                else None,  # 流通市值(亿->元)
                total_mv=(safe_float(fields[45]) or 0.0) * 100000000
    ...
    def _get_hk_realtime_quote(self, stock_code: str) -> Optional[UnifiedRealtimeQuote]:
        ...
        if circuit_breaker.is_available(em_key):
            try:
                ...
                if row.empty:
                    logger.info(
                        f"[API返回] 未找到港股 {code} 的实时行情 (stock_hk_spot_em)"
                    )
                else:
                    row = row.iloc[0]
                    quote = UnifiedRealtimeQuote(
        ...
        try:
            ...
            api_elapsed = _time.time() - api_start
            logger.info(
                f"[API返回] ak.stock_hk_spot 成功: 返回 {len(df_spot)} 只港股, 耗时 {api_elapsed:.2f}s"
            )
            ...
            logger.info(
                f"[港股实时行情-备用] {stock_code} {quote.name}: 价格={quote.price}, 涨跌={quote.change_pct}%"
            )
            return quote
    ...
    def _get_eastmoney_hot_up_stocks(
        ...
        for _, row in df.head(n).iterrows():
            rows.append(
                {
                    "rank": self._safe_int(row.get(rank_col))
                    if rank_col
                    else len(rows) + 1,
                    ...
                    "change_pct": self._safe_float(row.get(change_col))
                    if change_col
                    else None,
                    "source": "东方财富飙升榜",
                }
    ...
    def get_limit_up_pool(
        ...
        try:
            ...
            logger.info(
                "[API调用] ak.stock_zt_pool_em(date=%s) 获取涨停池...", query_date
            )
            ...
            if "首次封板时间" in df.columns:
                df["首次封板时间"] = df["首次封板时间"].map(
                    self._normalize_limit_time_value
                )
                df["_首次封板时间排序"] = df["首次封板时间"].where(
                    df["首次封板时间"] != "", "999999"
                )
            sort_cols = [
                col for col in ("连板数", "_首次封板时间排序") if col in df.columns
            ]
            if sort_cols:
                ...
                rows.append(
                    {
                        ...
                        "last_limit_time": self._normalize_limit_time_value(
                            row.get("最后封板时间")
                        ),
    ...
    @staticmethod
    def _find_column_containing(
        df: pd.DataFrame, keywords: Tuple[str, ...]
    ) -> Optional[str]:
